...
class CodeIngestor:
    def __init__(self,local_path: Optional[str] = None,github_repo: Optional[str] = None,github_token: Optional[str] = None,github_branch: str = "main",max_file_size: int = MAX_FILE_SIZE_BYTES,):
    ...
    def _walk_github(self) -> Iterator[dict]:
        ...
        for node in blobs:
            ...
            source_code = self._fetch_file_content(path)
            if source_code is None:
                continue
    ...
    def _fetch_file_content(self, path: str) -> Optional[str]:
        ...
        if encoding != "base64":
            logger.warning(f"Unexpected encoding '{encoding}' for {path}, skipping.")
            return None
        try:
            ...
            try:
                return base64.b64decode(raw_content).decode("latin-1")
            except Exception as e:
                logger.warning(f"Decode error for {path}: {e}")
                return None
        except Exception as e:
